...
def mapFromClm(clm, nside, sph_complex=False):
    """
    Given an array of C_{lm} values, produce a pixel-power-map (non-Nested) for
    healpix pixelation with nside
    ...
    @param clm:     Array of C_{lm} values (inc. 0,0 element)
    @param nside:   Nside of the healpix pixelation
    @param sph_complex: use complex spherical harmonics
    ...
    maxl = int(np.sqrt(len(clm))) - 1
    if sph_complex == True:
        h = np.zeros(npixels, dtype='complex_')
        for ll in range(maxl + 1):
            for mm in range(-ll, ll + 1):
                h += clm[ind] * ss.sph_harm(mm, ll, pixels[1], pixels[0])
                ind += 1
...
def getCov(clm, nside, F_e, sph_complex=False):
    """
    ...
    if sph_complex==True:
        sh00 = mapFromClm(clm, nside, sph_complex=sph_complex)
...
def anis_basis(psr_locs, lmax, nside=32, sph_complex = False):
    """
    ...
    for ll in range(0, lmax + 1):
        for mm in range(-ll, ll + 1):
            ...
            basis.append(getCov(clm, nside, F_e, sph_complex=sph_complex))
            clmindex += 1
