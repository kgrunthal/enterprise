...
@function
def aniscomplex_orf(pos1, pos2, params, **kwargs):
    """Anisotropic GWB spatial correlation function."""

    R_lm = kwargs["R_matrices"]
    psrs_pos = kwargs["psrs_pos"]
    clm = params
    if np.all(pos1==pos2):
        return 1. +0.j
        ...
        npsr = len(psrs_pos)
        k = int((npsr*(npsr-1)/2) - (npsr-a)*((npsr-a)-1)/2 + b - a - 1)
        #print(a, b, k)
        gamma_ab = np.matmul(R_lm[k], clm)
        return gamma_ab
